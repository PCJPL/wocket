...
module elliptical_nose_cone(wall_thickness, diameter=5, height=10, s=500) {
    // ELLIPTICAL NOSE CONE:
    //
    // Formula: y = R * sqrt(1 - pow((x / height), 2));
    
    module elliptical_cone(diameter, height, s) {
        ...
        for (i = [1 : s]){
            ...
            x_last = height * (i - 1) * inc;
            x = height * i * inc;

            y_last = diameter/2 * sqrt(1 - pow((x_last/height), 2));
            y = diameter/2 * sqrt(1 - pow((x/height), 2));

            rotate([0,0,90]) 
            polygon(points = [[0, y_last], [0, y], [x, y], [x_last, y_last]], convexity = 10);
        }
    }
    
    difference() {
        elliptical_cone(diameter, height, s);
        elliptical_cone(diameter-wall_thickness, height-wall_thickness, 100);
    }
}

module cone__sec(rho = 8, R = 5, L = 10, s = 500){
...
module tangent_ogive_nose_cone(wall_thickness, diameter, height, s){
    
    module ogive_cone(diameter, height, s) {
        rho = (pow(diameter/2,2) + pow(height,2)) / (2 * diameter/2);
        ...
        for (i = [1 : s]){
            ...
            y_last = sqrt(pow(rho,2) - pow((height - x_last), 2)) + diameter/2 - rho;

            y = sqrt(pow(rho,2) - pow((height - x), 2)) + diameter/2 - rho;
            ...
            polygon(points = [
                ...
                [x - height, 0], 
                [x - height, y], 
                [x_last - height, y_last]
            ], convexity = 10);
        }
    }
    
    difference() {
        ogive_cone(diameter, height, s);
        ogive_cone(diameter-wall_thickness, height-wall_thickness, 100);
    }
}
...
module conical_nose(wall_thickness, height, diameter) {

    translate([0,0,shoulder_height])
    difference() {
        cylinder(h = length, d1 = diameter, d2 = 0.4, center = false);
        cylinder(h = length-wall_thickness, d1 = diameter -wall_thickness, d2 = 0, center = false);
   

    }

    difference() {
        cylinder(h=shoulder_height+wall_thickness, d=shoulder_diameter);
        cylinder(h=shoulder_height, d=shoulder_diameter-wall_thickness);
    }
    
    cylinder(h=wall_thickness, d=shoulder_diameter);
}


module nose_cone(type, wall_thickness, diameter, height, s) {
    if (type=="Conical") {
        conical_nose_cone(wall_thickness, diameter, height, s);
    } 
    else if (type=="Elliptical") {
        elliptical_nose_cone(wall_thickness, diameter, height, s);
    }
    else if (type=="Tangent Ogive") {
        tangent_ogive_nose_cone(wall_thickness, diameter, height, s);
    }
}